...
## Собрать образ Docker с присвоением имени

docker build -t stocks_products .

## Показать доступные локальные образы

docker images

## Запустить контейнер Docker с присвоением имени, экспортировать порт контейнера

docker run --name stocks -p 7707:7707 stocks_products

## Показать работающие контейнеры

docker ps

## Посмотреть существующие на хосте контейнеры Docker
